...
impl UserData for DirsBridge {
    fn add_methods<'lua, M: UserDataMethods<'lua, Self>>(methods: &mut M) {
        ...
        methods.add_method("data_dir", |_, _, ()| {
            Ok(dirs::data_dir().map(|p| p.to_string_lossy().to_string()))
        });

        methods.add_method("cache_dir", |_, _, ()| {
            Ok(dirs::cache_dir().map(|p| p.to_string_lossy().to_string()))
        });

        methods.add_method("tools_dir", |_, _, ()| {
            Ok(dirs::tools_dir().map(|p| p.to_string_lossy().to_string()))
        });

        methods.add_method("scripts_dir", |_, _, ()| {
            Ok(dirs::scripts_dir().map(|p| p.to_string_lossy().to_string()))
        });

        methods.add_method("logs_dir", |_, _, ()| {
            Ok(dirs::logs_dir().map(|p| p.to_string_lossy().to_string()))
        });
    }
}
