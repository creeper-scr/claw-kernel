...
pub trait MessageFormat: Send + Sync {
    ...
    fn parse_stream_chunk(chunk: Self::StreamChunk) -> Option<Delta>;  // No 参数类型错误
}
...
|------|-------------|--------------|---------------|---------------|
| `register` | `Result<(), RegistryError>` | `Result<(), RegistryError>` | No 无Result | `Result<(), RegistryError>` |
| `execute` | `Result<ToolResult>` | `Result<ToolResult, ToolError>` | - | `Result<ToolResult, ToolError>` |
| `enable_hot_reload` | `HotReloadConfig` | `HotLoadingConfig` | No 无参数 | `HotLoadingConfig` |
...
// crate-map.md (错误)
async fn request<F: MessageFormat>(...) -> Result<CompletionResponse>;  // No 缺少错误类型
```
...
```rust
// crate-map.md
fn compile(...) -> Result<Script>;  // No 无错误类型

// BUILD_PLAN.md
fn compile(...) -> Result<Script, CompileError>;  // Yes 明确错误类型
```
