...
**Feature flags.** Use for optional heavy dependencies (`engine-v8`). Default build must have minimal dependencies.
...
| complete | all | Everything | ~35 min | ~100 MB |
...
> **项目状态**：v1.0.0 已发布。9 个 crate 已实现，670+ 个测试通过。现在是参与新 Provider、沙箱加固或测试覆盖的好时机。
...
**功能标志。** 对重量级可选依赖使用功能标志（`engine-v8`），默认构建必须具有最少依赖。
