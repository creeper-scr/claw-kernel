# Changelog

All notable changes to this project will be documented in this file.

The format is based on [Keep a Changelog](https://keepachangelog.com/en/1.0.0/),
and this project adheres to [Semantic Versioning](https://semver.org/spec/v2.0.0.html).

## [Unreleased]

### Note

This project is currently in the **design and planning stage**. No code has been implemented yet. See [ROADMAP.md](./ROADMAP.md) for planned features.

---

## [0.1.0] - TBD

### Added
- Initial release (planned)
- Core kernel crates
- Cross-platform sandbox support
- Script runtime with hot-loading
- LLM provider integrations

[Unreleased]: https://github.com/claw-project/claw-kernel/compare/v0.1.0...HEAD
[0.1.0]: https://github.com/claw-project/claw-kernel/releases/tag/v0.1.0
